...
<h3>取り扱い商品一覧</h3>
<!-- テキスト検索 -->
<form action="<%=request.getContextPath() %>/admin/serchCinema/title">
...
<!-- 値段検索 -->
<form action="<%=request.getContextPath() %>/admin/serchCinema/price" name="searchCinemaPriceForm">
...
<!-- ジャンル検索 -->
<form action="<%=request.getContextPath() %>/admin/serchCinema/genre" name="searchCinemaGenreForm">	
...
<!-- メディアタイプ検索 -->
<form action="<%=request.getContextPath() %>/admin/serchCinema/mediaType" name="searchCinemaMediaTypeForm">	
	...
	<tbody>
		...
		<c:forEach var="child" items="${listPage.childPageList}" varStatus="status">
				<th>
						...
						<img src="<%=request.getContextPath() %>/img/${child.imagePath} "class="img-responsive img-rounded" width="100" height="300">
						</c:if>
						<c:if test="${child.imagePath.length() > 60}">
